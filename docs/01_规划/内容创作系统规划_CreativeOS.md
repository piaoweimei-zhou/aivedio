...
> v1.6 更新：**P-INTEGRATION-1「内容资产最后一公里」完成**——`to_contract` v2 把 L3 分镜 prompt（含角色外观+镜头语言）组装为每段 `visual_hint`，storyboard(16字段含焊接点)+角色卡经 contract 透传 director 成片，修复"画面 prompt 永远 fallback 台词"的关键断点，端到端批次入库验证通过（e72afaa/e906e70）
> v1.7 更新：**P-INTEGRATION-2 五项能力接入 generate 默认流程（16d74d6）**——#14题材SKILL自动注入 / #10剧集记忆自动读前情提要+沉淀 / #11视觉特征角色卡→视觉一致块 / #7跨集连贯性自动检查 / #19合规门禁block级违规词就地替换；真实验证两集连续生成（记忆沉淀2条+连贯80/100+分镜含角色外观）
