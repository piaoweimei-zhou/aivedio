...
refreshIcons();
// 沙箱安全的 alert 替代：iframe sandbox 禁止 alert()，用 toast 或 console.warn 替代
function safeAlert(msg){
    try {
        ...
        if(el){
            ...
            clearTimeout(safeAlert._timer);
            safeAlert._timer = setTimeout(() => el.classList.remove('show'), 5000);
            return;
        }
    } catch(e){}
...
function applyTheme(theme){
    ...
    // 确保 light 时移除所有暗色 class
    if(theme === 'light'){
        ['theme-dark','theme-warm','studio-theme-dark'].forEach(c => {
            document.documentElement.classList.remove(c);
            document.body.classList.remove(c);
            shell.classList.remove(c);
        });
    }
    // dark 也兼容 studio-theme-dark
    document.documentElement.classList.toggle('studio-theme-dark', theme === 'dark');
    document.body.classList.toggle('studio-theme-dark', theme === 'dark');
    shell.classList.toggle('studio-theme-dark', theme === 'dark');
    // 更新主题按钮图标
    updateThemeButtonIcon(theme);
}
function cycleTheme(){
    const current = localStorage.getItem('studio_theme') || localStorage.getItem(CANVAS_THEME_KEY) || 'light';
    ...
    applyTheme(next);
    safeAlert({light:'浅色主题', dark:'暗色主题', warm:'暖色主题'}[next]);
}
function updateThemeButtonIcon(theme){
    const btn = document.getElementById('themeToggleBtn');
    if(!btn) return;
    const iconMap = {light:'sun', dark:'moon', warm:'sunset'};
    ...
    const labelMap = {light:'浅色', dark:'暗色', warm:'暖色'};
    const span = btn.querySelector('span');
    if(span) span.textContent = labelMap[theme] || '主题';
}
...
// D3 修复：页面关闭/隐藏时立即保存，防止防抖延迟导致生成结果丢失
window.addEventListener('beforeunload', () => {
    if(localCanvasDirty && !savingCanvasNow){
        clearTimeout(saveTimer);
        try { saveCanvas(); } catch(e) { /* 同步保存可能失败，忽略 */ }
    }
});
document.addEventListener('visibilitychange', () => {
    if(document.visibilityState === 'hidden' && localCanvasDirty){
        clearTimeout(saveTimer);
        try { saveCanvas(); } catch(e) {}
    }
});
...
async function runMsGenNode(nodeId, opts={}){
    ...
    try {
        ...
        refreshRunNodes(node, out);
        // D2 修复：从 MS 任务结果读取 asset_id 写入节点
        const msAssetId = results.find(r => r?.asset_id)?.asset_id;
        if(msAssetId){
            node.asset_id = msAssetId;
            node.metadata = node.metadata || {};
            node.metadata.asset_id = msAssetId;
            node.metadata.urls = outputUrls.map(outputUrlValue).filter(Boolean);
        }
        scheduleSave();
    } catch(err){
        const metas = collectRunMetas(out, pendingIds);
...
function comfyFieldKind(f){
    ...
    if(f.type === 'textarea' || /prompt|text|提示词|正向|负向|prompt/i.test(key)) return 'prompt';
    return 'setting';
}
...
async function runRhNode(nodeId, opts={}){
    ...
    try {
        ...
        node.runError = '';
        refreshRunNodes(node, out);
        // D2 修复：从 RH 任务结果读取 asset_id 写入节点
        if(result?.asset_id){
            node.asset_id = result.asset_id;
            node.metadata = node.metadata || {};
            node.metadata.asset_id = result.asset_id;
            node.metadata.urls = outputs.map(outputUrlValue).filter(Boolean);
        }
        scheduleSave();
    } catch(err) {
        const meta = collectRunMeta(out, pendingId);
...
function renderComfySettings(container, node){
    ...
    if(mode === 'text'){
        ...
        // 按分组组织字段
        let fieldsHtml = '';
        if(groups.length > 0 && fields.some(f => f.group)){
            // 有分组配置：按分组渲染
            for(const g of groups){
                const gFields = fields.filter(f => f.group === g.id);
                if(!gFields.length) continue;
                const isGrid = g.layout === 'grid';
                fieldsHtml += `<div class="comfy-field-group" data-group="${escapeHtml(g.id)}">`;
                fieldsHtml += `<div class="comfy-group-title">${escapeHtml(g.name || g.id)}</div>`;
                if(isGrid){
                    // 网格布局：紧凑排列图片
                    fieldsHtml += `<div class="comfy-grid-row">`;
                    for(const f of gFields){
                        fieldsHtml += renderComfyCustomField(node, f, true);
                    }
                    fieldsHtml += `</div>`;
                } else {
                    for(const f of gFields){
                        fieldsHtml += renderComfyCustomField(node, f, false);
                    }
                }
                fieldsHtml += `</div>`;
            }
            ...
            if(imageFields.length){
                fieldsHtml += `<div class="comfy-field-group" data-group="images"><div class="comfy-group-title">参考图片</div><div class="comfy-grid-row">`;
                for(const f of imageFields) fieldsHtml += renderComfyCustomField(node, f, true);
                fieldsHtml += `</div></div>`;
            }
            if(promptFields.length){
                fieldsHtml += `<div class="comfy-field-group" data-group="prompts"><div class="comfy-group-title">分镜提示词</div>`;
                for(const f of promptFields) fieldsHtml += renderComfyCustomField(node, f, false);
                fieldsHtml += `</div>`;
            }
            for(const f of settingFields) fieldsHtml += renderComfyCustomField(node, f, false);
        }
        ...
        container.innerHTML = `
            ...
            ${data ? fieldsHtml : ''}
            ${data && !hasAnyField ? `<div class="text-[11px] text-slate-400">${tr('canvas.comfyNoExtraParams')}</div>` : ''}
        `;
    ...
    // 图片上传按钮
    container.querySelectorAll('.comfy-img-upload-btn').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = e => {
            e.stopPropagation();
            const fieldId = btn.dataset.comfyImageUpload;
            ...
            fileInput.onchange = async () => {
                ...
                try {
                    const res = await fetch('/api/comfyui/upload/image', {method:'POST', body: formData});
                    if(!res.ok) {
                    ...
                    node.comfyParams = node.comfyParams || {};
                    node.comfyParams[fieldId] = filename;
                    // 保存预览 URL 用于缩略图显示
                    node.comfyParamPreviews = node.comfyParamPreviews || {};
                    node.comfyParamPreviews[fieldId] = `/api/comfyui/image?filename=${encodeURIComponent(filename)}`;
                    render();
                    scheduleSave();
                    // 同步注册到资产库（不阻塞主流程）
                    try {
                        const assetName = file.name.replace(/\.[^.]+$/, '') || filename;
                        await fetch('/api/director/assets', {
                            method: 'POST',
                            headers: {'Content-Type': 'application/json'},
                            body: JSON.stringify({
                                asset_type: 'concept',
                                name: assetName,
                                urls: [`/api/comfyui/image?filename=${encodeURIComponent(filename)}`],
                                metadata: {source: 'canvas_upload', filename},
                                content_type: '',
                            })
                        });
                    } catch(assetErr) {
                        console.warn('[Canvas] 资产库注册失败（不影响使用）:', assetErr);
                    }
                } catch(err) {
                    console.error('Image upload failed:', err);
                    safeAlert('图片上传失败: ' + (err.message || ''));
                }
            };
            fileInput.click();
        };
    ...
    // 图片选择按钮（从资产库）
    container.querySelectorAll('.comfy-img-pick-btn').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = e => {
            e.stopPropagation();
            const fieldId = btn.dataset.comfyImagePick;
            // 从资产库 API 获取图片资产
            Promise.all([
                fetch('/api/director/assets').then(r => r.json()).then(d => d.assets || []).catch(() => []),
                fetch('/api/local-assets').then(r => r.json()).then(d => d.items || []).catch(() => [])
            ]).then(([assets, localItems]) => {
                // 合并资产库图片和本地上传文件，去重
                const seen = new Set();
                const items = [];
                // 优先显示资产库中的图片（有缩略图和名称）
                assets.filter(a => a.urls && a.urls.length && a.asset_type !== 'video').forEach(a => {
                    const url = a.urls[0];
                    const name = a.name || url.split('/').pop().split('?')[0] || '';
                    const key = name.toLowerCase();
                    if(!seen.has(key)){
                        seen.add(key);
                        items.push({name, url, source: 'library', asset_type: a.asset_type, asset_id: a.asset_id});
                    }
                });
                // 补充本地上传文件
                localItems.filter(i => i.kind === 'image').forEach(i => {
                    const key = i.name.toLowerCase();
                    if(!seen.has(key)){
                        seen.add(key);
                        items.push({name: i.name, url: i.url, source: 'local'});
                    }
                });
                if(!items.length) { safeAlert('资产库中没有图片，请先上传'); return; }
                // 创建选择弹窗
                const overlay = document.createElement('div');
                overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;display:flex;align-items:center;justify-content:center';
                const panel = document.createElement('div');
                panel.style.cssText = 'background:#1e293b;border-radius:12px;padding:16px;max-width:520px;width:90%;max-height:70vh;overflow-y:auto';
                panel.innerHTML = `<div style="color:#e2e8f0;font-size:14px;font-weight:bold;margin-bottom:12px">从资产库选择图片</div>
                    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(90px,1fr));gap:8px" id="assetPickGrid"></div>`;
                const grid = panel.querySelector('#assetPickGrid');
                items.forEach(item => {
                    const card = document.createElement('div');
                    card.style.cssText = 'cursor:pointer;border-radius:8px;overflow:hidden;background:#0f172a;border:1px solid #334155;transition:border-color .15s';
                    card.onmouseenter = () => card.style.borderColor = '#60a5fa';
                    card.onmouseleave = () => card.style.borderColor = '#334155';
                    const typeLabel = item.source === 'library' ? (item.asset_type || '') : '本地';
                    card.innerHTML = `<img src="${item.url}" style="width:100%;aspect-ratio:1;object-fit:cover;display:block" onerror="this.style.display='none'">
                        <div style="padding:4px 6px;font-size:10px;color:#94a3b8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${item.name}</div>
                        ${typeLabel ? `<div style="padding:0 6px 4px;font-size:8px;color:#60a5fa">${typeLabel}</div>` : ''}`;
                    card.onclick = () => {
                        // 从 URL 中提取 ComfyUI 文件名，同时保存预览 URL
                        const url = item.url || '';
                        let comfyFilename = '';
                        // 尝试从 /api/comfyui/image?filename=xxx 提取
                        const m = url.match(/filename=([^&]+)/);
                        if(m) comfyFilename = decodeURIComponent(m[1]);
                        // 否则从 URL 路径提取文件名
                        if(!comfyFilename) comfyFilename = url.split('/').pop().split('?')[0] || item.name;
                        // 如果文件名没有扩展名，尝试从 URL 推断
                        if(comfyFilename && !/\.\w{2,4}$/.test(comfyFilename)){
                            const ext = url.match(/\.(png|jpg|jpeg|webp|gif|bmp)/i);
                            if(ext) comfyFilename += '.' + ext[1].toLowerCase();
                        }
                        node.comfyParams = node.comfyParams || {};
                        node.comfyParams[fieldId] = comfyFilename;
                        // 保存预览 URL 用于缩略图显示
                        node.comfyParamPreviews = node.comfyParamPreviews || {};
                        node.comfyParamPreviews[fieldId] = url;
                        overlay.remove();
                        render();
                        scheduleSave();
                    };
            ...
            }).catch(err => { console.error('Failed to list assets:', err); safeAlert('资产库加载失败'); });
        };
    });
    // 图片清除按钮
    container.querySelectorAll('.comfy-img-clear-btn').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = e => {
            e.stopPropagation();
            const fieldId = btn.dataset.comfyImageClear;
            node.comfyParams = node.comfyParams || {};
            delete node.comfyParams[fieldId];
            node.comfyParamPreviews = node.comfyParamPreviews || {};
            delete node.comfyParamPreviews[fieldId];
            render();
            scheduleSave();
        };
    });
}
function renderComfyCustomField(node, f, compact){
    // sameAs: 自动跟随另一个字段的值
    let value = comfyParamValue(node, f);
    const sameAsField = f.sameAs || null;
    const sameAsValue = sameAsField ? comfyParamValue(node, {id: sameAsField}) : null;
    const isSameAs = sameAsField && !value && sameAsValue;
    if(isSameAs) value = sameAsValue; // 显示源字段的值
    const label = escapeHtml(f.name || f.input);
    const desc = f.description ? `<div class="field-desc">${escapeHtml(f.description)}</div>` : '';
    const optional = f.optional ? `<span class="field-optional">（可选）</span>` : '';
    const sameAsTag = sameAsField ? `<span class="field-sameas" data-sameas="${escapeHtml(sameAsField)}">同上</span>` : '';
    if(f.type === 'image'){
        // 优先使用预览 URL（资产库图片可能不是 ComfyUI 格式），回退到 ComfyUI image API
        // sameAs 场景：也检查源字段的预览 URL
        let previewUrl = (node.comfyParamPreviews || {})[f.id] || '';
        if(!previewUrl && isSameAs && sameAsField) previewUrl = (node.comfyParamPreviews || {})[sameAsField] || '';
        const imgSrc = value ? (previewUrl || `/api/comfyui/image?filename=${encodeURIComponent(value)}`) : '';
        const hasValue = !!value;
        if(compact){
            // 紧凑模式：小缩略图 + 名称 + 按钮，适合 grid 布局
            const preview = imgSrc ? `<img src="${imgSrc}" class="comfy-img-thumb" onerror="this.style.display='none'">` : `<div class="comfy-img-placeholder">无图片</div>`;
            const clearBtn = hasValue ? `<button type="button" class="comfy-img-clear-btn" data-comfy-image-clear="${escapeHtml(f.id)}" title="清除图片">✕</button>` : '';
            return `<div class="comfy-img-card${isSameAs ? ' sameas-active' : ''}" title="${escapeAttr(f.description || '')}">
                ${preview}
                ${clearBtn}
                <div class="comfy-img-label">${label}${optional}${sameAsTag}</div>
                <div class="comfy-img-actions">
                    <button type="button" class="comfy-img-upload-btn" data-comfy-image-upload="${escapeHtml(f.id)}" title="上传图片">上传</button>
                    <button type="button" class="comfy-img-pick-btn" data-comfy-image-pick="${escapeHtml(f.id)}" title="从资产库选择">选择</button>
                </div>
                <input type="hidden" data-comfy-param="${escapeHtml(f.id)}" data-comfy-type="image" value="${escapeHtml(value)}" ${isSameAs ? 'data-sameas-value' : ''}>
            </div>`;
        }
        const preview = imgSrc ? `<img src="${imgSrc}" style="max-width:100%;max-height:80px;border-radius:4px;margin-top:4px;object-fit:contain" onerror="this.style.display='none'">` : '';
        const clearBtn = hasValue ? `<button type="button" class="comfy-img-clear-btn" data-comfy-image-clear="${escapeHtml(f.id)}" style="font-size:10px;padding:2px 6px;border-radius:4px;background:#7f1d1d;color:#fca5a5;border:1px solid #991b1b;cursor:pointer;white-space:nowrap" title="清除图片">清除</button>` : '';
        return `<div class="gen-settings-row" style="flex-wrap:wrap">
            <label class="field" style="flex:1;min-width:120px"><div class="setting-title">${label}${optional}${sameAsTag}</div>${desc}
            <div style="display:flex;gap:4px;align-items:center">
                <input type="text" class="setting-input" data-comfy-param="${escapeHtml(f.id)}" data-comfy-type="image" placeholder="图片文件名..." value="${escapeHtml(value)}" ${isSameAs ? 'data-sameas-value' : ''} style="font-size:11px;flex:1">
                <button type="button" class="comfy-img-upload-btn" data-comfy-image-upload="${escapeHtml(f.id)}" style="font-size:10px;padding:2px 6px;border-radius:4px;background:#334155;color:#e2e8f0;border:1px solid #475569;cursor:pointer;white-space:nowrap" title="上传图片">上传</button>
                <button type="button" class="comfy-img-pick-btn" data-comfy-image-pick="${escapeHtml(f.id)}" style="font-size:10px;padding:2px 6px;border-radius:4px;background:#334155;color:#e2e8f0;border:1px solid #475569;cursor:pointer;white-space:nowrap" title="从资产库选择">选择</button>
                ${clearBtn}
            </div>
            ${preview}
            </label>
    ...
    if(f.type === 'textarea'){
        return `<div class="gen-settings-row">
            <label class="field" style="flex:1"><div class="setting-title">${label}${optional}</div>${desc}<textarea class="setting-input" data-comfy-param="${escapeHtml(f.id)}" data-comfy-type="textarea" style="height:66px;padding-top:8px;resize:vertical" placeholder="${escapeAttr(f.description || '')}">${escapeHtml(value)}</textarea></label>
        </div>`;
    }
...
async function runVideoNode(nodeId, opts={}){
    ...
    try {
        ...
        node.runStatus = 'done'; node.runError = '';
        refreshRunNodes(node, out);
        // D2/D5 修复：从视频任务结果读取 asset_id 写入节点
        if(result?.asset_id){
            node.asset_id = result.asset_id;
            node.metadata = node.metadata || {};
            node.metadata.asset_id = result.asset_id;
...
async function runComfyUpscale(imageUrl, resolution){
    ...
    const submitUpscale = await fetch('/api/generate', {
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({
    ...
    let upscale = submitUpscale;
    if(submitUpscale.task_id && submitUpscale.status === 'pending'){
        upscale = await pollGenerateTask(submitUpscale.task_id, '', null);
    }
...
async function runLTXDirectorNode(nodeId, opts={}){
    ...
    try {
        ...
        const submitLtx = await cascadeFetch('/api/generate', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({
                prompt:globalPrompt,
        ...
        let result = submitLtx;
        if(submitLtx.task_id && submitLtx.status === 'pending'){
            result = await pollGenerateTask(submitLtx.task_id, cascadeTargetId, run);
        }
        run.request = requestMetaFromResult(result);
        ...
        node.runError = '';
        refreshRunNodes(node, out);
        // D2 修复：从 LTX 任务结果读取 asset_id 写入节点
        if(result?.asset_id){
            node.asset_id = result.asset_id;
            node.metadata = node.metadata || {};
            node.metadata.asset_id = result.asset_id;
...
async function pollGenerateTask(taskId, cascadeTargetId, run){
    /** 轮询异步生成任务状态，直到完成或失败 */
    const POLL_INTERVAL = 3000;  // 3 秒轮询一次
    const MAX_POLL_TIME = 1800000; // 最长 30 分钟
    const startTime = Date.now();
    let lastStatus = 'pending';
    while(Date.now() - startTime < MAX_POLL_TIME){
        await new Promise(r => setTimeout(r, POLL_INTERVAL));
        try {
            const resp = await cascadeFetch(`/api/generate/${taskId}`, {}, {cascadeTargetId});
            const data = await resp.json();
            if(data.status === 'succeeded'){
                return data;
            } else if(data.status === 'failed'){
                return {images: [], error: data.error || '生成失败', success: false};
            }
            // 更新进度提示
            if(run && data.status !== lastStatus){
                lastStatus = data.status;
                if(run.taskLabel) run.taskLabel = `${tr('canvas.generating') || '生成中'}... (${data.status})`;
            }
        } catch(err) {
            // 轮询失败不中断，继续重试
            console.warn('[pollGenerateTask] 轮询失败，继续重试:', err.message);
        }
    }
    return {images: [], error: '生成超时（30分钟）', success: false};
}
...
async function runComfyNode(nodeId, opts={}){
    ...
    const requiredImageFields = customImageFields.filter(f => !f.optional);  // 排除可选字段
    const requiredVideoFields = customVideoFields.filter(f => !f.optional);
    const requiredAudioFields = customAudioFields.filter(f => !f.optional);
    const hasPromptInField = mode === 'custom' && customPromptFields.some(f => comfyParamValue(node, f));
    if((mode === 'text' || (mode === 'custom' && customPromptFields.length && !hasPromptInField)) && !prompt){ safeAlert(tr('canvas.needPrompt')); return; }
    const imageInParamsCount = mode === 'custom' ? customImageFields.filter(f => comfyParamValue(node, f)).length : 0;
    const requiredImageFilled = mode === 'custom' ? requiredImageFields.every(f => comfyParamValue(node, f)) : true;
    if(mode === 'custom'){
        console.log('[runComfyNode] custom mode validation:', {
            refs: refs.length,
            imageFields: customImageFields.length,
            requiredImageFields: requiredImageFields.length,
            optionalImageFields: customImageFields.filter(f => f.optional).length,
            imageInParams: imageInParamsCount,
            requiredImageFilled,
            comfyParams: JSON.stringify(node.comfyParams),
            fieldValues: customImageFields.map(f => ({id: f.id, val: comfyParamValue(node, f), optional: f.optional})),
        });
    }
    if((mode !== 'text' && mode !== 'custom' && !refs.length) || (mode === 'custom' && !requiredImageFilled)){ safeAlert(tr('canvas.needImage')); return; }
    if(mode === 'custom' && videoRefsOnly(allRefs).length < requiredVideoFields.length){ safeAlert(langIsEn() ? 'Please connect enough video inputs for this ComfyUI workflow.' : '请为这个 ComfyUI 工作流连接足够的视频输入'); return; }
    if(mode === 'custom' && audioRefsOnly(allRefs).length < requiredAudioFields.length){ safeAlert(langIsEn() ? 'Please connect enough audio inputs for this ComfyUI workflow.' : '请为这个 ComfyUI 工作流连接足够的音频输入'); return; }
    ...
    try {
        ...
        if(mode === 'text'){
            ...
            const submitText = await cascadeFetch('/api/generate', {
                method:'POST',
                headers:{'Content-Type':'application/json'},
                body:JSON.stringify({
                    prompt,
            ...
            if(submitText.task_id && submitText.status === 'pending'){
                result = await pollGenerateTask(submitText.task_id, cascadeTargetId, run);
            }
            run.request = requestMetaFromResult(result);
            ...
            const submitEnhance = await cascadeFetch('/api/generate', {
                method:'POST',
                headers:{'Content-Type':'application/json'},
                body:JSON.stringify({
            ...
            let enhance = submitEnhance;
            if(submitEnhance.task_id && submitEnhance.status === 'pending'){
                enhance = await pollGenerateTask(submitEnhance.task_id, cascadeTargetId, run);
            }
            ...
            const assignMediaFields = async (mediaFields, mediaRefs) => {
                ...
                mediaFields.forEach((f, i) => {
                    ...
                    // 优先使用 comfyParams 中上传的值，回退到 sameAs 源字段的值，再回退到 field.default
                    let val = comfyParamValue(node, f);
                    if(!val && f.sameAs){
                        val = comfyParamValue(node, {id: f.sameAs});
                    }
                    const finalVal = val || names[i] || '';
                    // 可选字段为空时跳过，不注入空值到工作流
                    if(f.optional && !finalVal) return;
                    params[f.node][f.input] = finalVal;
                });
            };
            ...
            promptFields.forEach(f => {
                if(!f.node || !f.input) return;
                params[f.node] = params[f.node] || {};
                params[f.node][f.input] = comfyParamValue(node, f) || prompt;
            });
            ...
            // 收集可选且为空的图片/视频/音频字段对应的节点 ID，后端跳过这些节点的图片检查
            const optionalEmptyNodes = [];
            for(const f of [...imageFields, ...videoFields, ...audioFields]){
                if(f.optional && !comfyParamValue(node, f) && !(f.sameAs && comfyParamValue(node, {id: f.sameAs}))){
                    if(f.node && !optionalEmptyNodes.includes(f.node)) optionalEmptyNodes.push(f.node);
                }
            }
            const submitResult = await cascadeFetch('/api/generate', {
                method:'POST',
                headers:{'Content-Type':'application/json'},
                body:JSON.stringify({
                    prompt,
                    ...
                    client_id:CLIENT_ID,
                    optional_empty_nodes:optionalEmptyNodes
                })
            ...
            // 如果返回了 task_id，进入异步轮询模式
            let result = submitResult;
            if(submitResult.task_id && submitResult.status === 'pending'){
                result = await pollGenerateTask(submitResult.task_id, cascadeTargetId, run);
            }
            run.request = requestMetaFromResult(result);
            if(result.error) throw new Error(actionFailed('canvas.comfyCustom', result.error));
            images = comfyResultOutputs(result);
            console.log('[runComfyNode] custom result:', {rawImages: result.images, parsedImages: images.length, outId: out?.id});
            ...
            const submitResult2 = await cascadeFetch('/api/generate', {
                method:'POST',
                headers:{'Content-Type':'application/json'},
                body:JSON.stringify({
                    prompt,
            ...
            let result2 = submitResult2;
            if(submitResult2.task_id && submitResult2.status === 'pending'){
                result2 = await pollGenerateTask(submitResult2.task_id, cascadeTargetId, run);
            }
            run.request = requestMetaFromResult(result2);
            if(result2.error) throw new Error(actionFailed('canvas.comfyEdit', result2.error));
            if(!result2.images?.length) throw new Error(noReturnedImage('canvas.comfyEdit'));
            images = node.editUpscale ? await runComfyUpscale(result2.images?.[0], node.editUpscaleRes || 2048) : result2.images || [];
        }
        const meta = collectRunMeta(out, pendingId);
        if(out) out._pending = (out._pending||[]).filter(p => p.id !== pendingId);
        appendOutputImages(out, images, refs[0], [meta]);
        mergeGeneratedOutputs(node, images, Boolean(opts.cascade));
        console.log('[runComfyNode] done:', {images: images.length, outImages: out?.images?.length, outId: out?.id, nodeStatus: node.runStatus});
        addGenerationLog({run, outputs:images, runMs:meta.runMs || 0});
        node.runStatus = 'done'; node.runError = '';
        refreshRunNodes(node, out);
        // D2 修复：后端 _execute_generate_task 已统一创建资产并返回 asset_id，
        // 这里从任务结果读取 asset_id 写入节点，避免前端重复创建导致资产重复
        if(result?.asset_id){
            node.asset_id = result.asset_id;
            node.metadata = node.metadata || {};
            node.metadata.asset_id = result.asset_id;
            node.metadata.urls = images.map(outputUrlValue).filter(Boolean);
        }
        scheduleSave();
    } catch(err) {
        const meta = collectRunMeta(out, pendingId);
        ...
        refreshRunNodes(node, out);
        if(opts.cascade) throw err;
        safeAlert(err.message || actionFailed('canvas.comfyGenerate'));
    }
}
...
function retryBarHtml(node){
    const isCascade = Boolean(node._cascadeFailed);
    const hasOutputs = (node.generatedOutputs || []).length > 0;
    if(node.runStatus === 'failed'){
        const errMsg = (node.runError || tr('canvas.generationFailed')).slice(0, 120);
        return `<div class="node-retry-bar" data-retry-bar>
            <span class="node-retry-msg" title="${escapeAttr(node.runError||'')}">${escapeHtml(errMsg)}</span>
            ${isCascade ? `<button class="node-retry-btn" type="button" data-retry="${node.id}">重试</button><button class="node-stop-btn" type="button" data-stop="${node.id}">停止</button>` : ''}
        </div>`;
    }
    if(hasOutputs && !node.running){
        return `<div class="node-retry-bar" data-retry-bar>
            <button class="node-retry-btn" type="button" data-regenerate="${node.id}"><i data-lucide="refresh-cw" class="w-3 h-3 inline-block mr-1"></i>重新生成</button>
        </div>`;
    }
    return '';
}
function bindCascadeButtons(wrap, nodeId){
    ...
    wrap.querySelectorAll(`[data-regenerate="${nodeId}"]`).forEach(b => {
        b.onmousedown = e => e.stopPropagation();
        b.onclick = e => { e.stopPropagation(); runCanvasGenerate(nodeId); };
    });
...
async function registerGeneratedOutputsAsAssets(images, node, run){
    /** 将生成结果图片注册到资产库，以便在资产页面显示 */
    if(!images || !images.length) return;
    const assetType = String(node?.type || 'image') === 'video' ? 'video' : 'storyboard';
    const nodeLabel = node?.label || node?.name || run?.taskLabel || '';
    let registeredCount = 0;
    for(const item of images){
        try {
            const url = outputUrlValue(item);
            if(!url) continue;
            const name = (item?.name || nodeLabel || 'generated').replace(/[^\w\u4e00-\u9fff\-_ ]/g, '').slice(0, 60) || 'generated';
            const res = await fetch('/api/director/assets', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({
                    ...
                    metadata: {
                        ...
                        node_type: node?.type || '',
                        workflow: node?.comfyWorkflow || '',
                        prompt: run?.prompt || '',
                    },
                    content_type: 'character',
                })
            });
            if(res && res.ok) registeredCount++;
        } catch(err) {
            console.warn('[Canvas] 生成结果注册资产库失败（不影响使用）:', err);
        }
    }
    // 通知父窗口（React 前端）资产已创建，以便刷新资产列表
    if(registeredCount > 0){
        try {
            window.parent?.postMessage({type: 'asset-created', count: registeredCount}, '*');
        } catch(e) {
            // 跨域或无父窗口时忽略
        }
    }
