...
class MsrVideoRequest(BaseModel):
    ...
    duration: Optional[float] = Field(None, ge=0.1, le=60, description="视频时长（秒），若提供则忽略 frame_count")
...
async def submit_msr_video(request: MsrVideoRequest):
    ...
    logger.info(f"[MSR] comfyui_dir={comfyui_svc.config.comfyui_dir} output_dir={comfyui_svc.config.output_dir}")
    ...
    ref_nodes = {"29": request.ref1_image_url, "40": request.ref2_image_url,
                 "33": request.ref3_image_url, "95": request.ref4_image_url,
    ...
    if "50" in wf_data:
        if request.duration:
            ...
            logger.info(f"[MSR] 注入 total_length={calc_total_frames} (duration={request.duration}s × fps={request.fps}) → 节点 50")
        else:
            ...
            if current_total < request.frame_count:
                ...
                logger.info(f"[MSR] total_length 调整为 {request.frame_count}（保证 ≥ frame_count）→ 节点 50")
    ...
    if "27" in wf_data and request.steps:
        ...
        if original_sigmas:
            ...
            if current_steps != request.steps:
                ...
                logger.info(f"[MSR] 注入 steps={request.steps} (重采样 {current_steps}→{request.steps}) → 节点 27")
            else:
                logger.info(f"[MSR] steps={request.steps} 与当前一致，无需重采样")
        else:
            logger.warning(f"[MSR] 节点 27 无 sigmas 字段，无法注入 steps")
    else:
                ...
                try:
                    ...
                    for url in actual_urls:
                        asset = await asset_svc.create(
                            asset_type="video",
                            name=f"MSR 多角色视频 {time.strftime('%m-%d %H:%M', time.localtime(time.time()))}",
                            urls=[url],
                            content_type="video",
                        )
                        if asset:
