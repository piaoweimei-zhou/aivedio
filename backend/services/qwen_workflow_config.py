    ...
    def __init__(self, config_path: Optional[str] = None, force_reinit: bool = False):
        ...
        if hasattr(self, '_initialized') and self._initialized and not force_reinit:
            ...
            if config_path and config_path != self.config_path:
                logger.warning(f"[QwenConfig] 配置路径已设置为 {self.config_path}，新路径 {config_path} 将被忽略。使用 force_reinit=True 强制重新初始化。")
            return
        ...
        return {
            ...
            'workflow': {
                ...
                'available_workflows': [
                    ...
                    {'name': "LTX2.3导演2", 'file': "LTX2.3导演2.json", 'description': "导演工作流"},
                    {'name': "Qwen3+TTS+音色设计", 'file': "Qwen3+TTS+音色设计.json", 'description': "TTS音色设计"},
                    {'name': "Qwen3+TTS+音频克隆", 'file': "Qwen3+TTS+音频克隆.json", 'description': "TTS音频克隆"},
                    {'name': "seed视频放大工作流", 'file': "seed视频放大工作流.json", 'description': "视频放大工作流"},
                ],
            },
            'modes': {
