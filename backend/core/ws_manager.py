...
class UnifiedWSManager:
    ...
    async def accept(
        self, ws: WebSocket, conn_id: str, channel: str = "chat", group: str = "", meta: dict = None
    ):  # noqa: E501
        """接受并注册 WebSocket 连接"""
        await ws.accept()
        ...
        logger.debug(
            f"[WS] {conn_id} → channel={channel} group={group} (active: {self.active_count}, group_conns: {_group_count})"  # noqa: E501
        )  # noqa: E501
    ...
    def disconnect(self, conn_id: str):
        ...
        _group_count = (
            len(_ch.groups.get(group, set())) if group and _ch and group in _ch.groups else 0
        )  # noqa: E501
        logger.debug(
            f"[WS] {conn_id} ✗ channel={channel} group={group} (active: {self.active_count}, group_conns: {_group_count})"  # noqa: E501
        )  # noqa: E501
        ...
        for conn_id in group_conns:
            ...
            if ws:
                try:
                    ...
                    dead.add(conn_id)
                    logger.debug(
                        f"[WS] 组播发送失败 channel={channel} group={group} conn={conn_id}: {e}"
                    )
            else:
                dead.add(conn_id)
        for conn_id in dead:
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "channels": {name: ch.count for name, ch in self.channels.items() if ch.count > 0},
        }
