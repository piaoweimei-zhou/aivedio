...
async def smoke():
    scope = [
        ("/health", "GET"),
        ("/api/director/projects", "GET"),
        ("/api/director/assets?limit=1", "GET"),  # noqa: E501
        ("/api/director/assets/types", "GET"),
        ("/api/director/assets/stage-types", "GET"),
        ("/api/director/assets/content-types", "GET"),
        ("/api/director/assets/stats/overview", "GET"),  # noqa: E501
        ("/api/director/stages", "GET"),
        ("/api/director/stages/script/video-types", "GET"),
        ("/api/director/stages/graphic/types", "GET"),
        ("/api/director/stages/tts/voices", "GET"),  # noqa: E501
        ("/api/director/stages/screen/windows", "GET"),
        ("/api/director/providers", "GET"),
        ("/api/director/providers/config/meta", "GET"),
        ("/api/director/providers/health/all", "GET"),  # noqa: E501
        ("/api/director/batches", "GET"),
        ("/api/director/workflow-templates", "GET"),
        ("/api/director/presets", "GET"),
        ("/api/director/prompts", "GET"),
        ("/api/director/prompts/categories", "GET"),
        ("/api/director/prompts/tags", "GET"),
        ("/api/director/prompts/stats", "GET"),
        ("/api/canvas/", "GET"),
        ("/api/config", "GET"),  # noqa: E501
        ("/api/workflows", "GET"),
        ("/api/canvases", "GET"),
        ("/api/local-assets", "GET"),
        ("/api/canvas-assets", "GET"),
        ("/api/asset-library", "GET"),
        ("/api/prompt-libraries", "GET"),
    ]  # noqa: E501
    for path, m in scope:
        r = await req(m, path)
        cat = (
            ...
            else ("EXPECTED_FAIL" if r.status_code < 500 else "UNEXPECTED")
        )  # noqa: E501
...
async def main():
    ...
    r, j = await jreq(
        "POST",
        "/api/director/workflow-templates",
        json={
            "name": "_selftest_wf",
            "steps": [{"stage_id": "concept", "input_mode": "upload"}],
            "required_inputs": [],
        },
    )  # noqa: E501
    ...
    r, j = await jreq(
        ...
        json={
            "asset_type": "concept",
            "name": "_selftest_asset",
            "project_id": pid,
            "urls": [],
            "metadata": {"src": "selftest"},
        },
    )  # noqa: E501
    aid = (
        (j or {}).get("asset", {}).get("asset_id")
        or (j or {}).get("asset_id")
        or (j or {}).get("id")
    )  # noqa: E501
    pr(cls(r.status_code), "CRUD", "Asset create", r.status_code, f"id={aid}")
    if aid:
        for ep, nm in [
            (f"/api/director/assets/{aid}/lineage", "asset lineage"),
            (f"/api/director/assets/{aid}/children", "asset children"),
        ]:  # noqa: E501
            r = await req("GET", ep)
    ...
    r, j = await jreq(
        "POST",
        "/api/director/prompts",
        json={
            "name": "_res_prompt",
            "content": "Hi {name} @ {place}",
    ...
    if p_id:
        r2, j2 = await jreq(
            "POST",
            f"/api/director/prompts/{p_id}/resolve",
            json={"variables": {"name": "Tom", "place": "HK"}},
        )  # noqa: E501
        ok = r2.status_code < 400 and "Tom" in json.dumps(j2, ensure_ascii=False)
        pr(
            "PASS" if ok else "UNEXPECTED",
            "LOGIC",
            "prompt resolve",
            r2.status_code,
            json.dumps(j2, ensure_ascii=False)[:120],
        )  # noqa: E501
    ...
    if se_id:
        for nm, mth, body in [
            ("preset set-default", "POST", f"/api/director/presets/{se_id}/set-default")
        ]:  # noqa: E501
    ...
    steps = [
        {
            ...
            "stage_id": "concept",
            "name": "c1",
            "input_asset_ids": [aid] if aid else [],
        }
    ]  # noqa: E501
    r, j = await jreq(
        "POST",
        "/api/director/batches",
        json={"name": "_b", "steps": steps, "project_id": pid, "stop_on_failure": False},
    )  # noqa: E501
    b_id = (j or {}).get("batch", {}).get("batch_id") or (j or {}).get("batch_id")
    if b_id:
        r = await req("GET", f"/api/director/batches/{b_id}/dag")
        pr(cls(r.status_code), "LOGIC", "batch dag", r.status_code)  # noqa: E501
        r = await req("POST", f"/api/director/batches/{b_id}/dry-run")
        pr(cls(r.status_code), "LOGIC", "batch dry-run", r.status_code, r.text[:120])  # noqa: E501

    # wf create-batch
    if wf_id:
        r = await req(
            "POST",
            f"/api/director/workflow-templates/{wf_id}/create-batch",
    ...
    if cid:
        r = await req(
            ...
            f"/api/canvas/{cid}",
            json={
                "name": "_selftest_canvas",
                "nodes": [{"node_id": "n1", "x": 10, "y": 20, "label": "a"}],
                "edges": [],
                "viewport": {"x": 0, "y": 0, "zoom": 1},
            },
        )  # noqa: E501
        pr(cls(r.status_code), "LOGIC", "canvas layout update", r.status_code, r.text[:120])
        r = await req("GET", f"/api/canvas/{cid}")
        pr(cls(r.status_code), "LOGIC", "canvas get", r.status_code)  # noqa: E501
        r, j = await jreq(
            "POST",
            f"/api/canvas/{cid}/nodes",
            json={"node_id": "nn1", "asset_id": aid, "node_type": "image", "x": 1, "y": 1},
        )  # noqa: E501
        pr(
            cls(r.status_code),
            "LOGIC",
    ...
    for stage, provider, inputs in [
        ("concept", "comfyui", [aid] if aid else []),
        ...
        r, j = await jreq(
            ...
            json={
                ...
                "input_asset_ids": inputs,  # noqa: E501
                "params": {"prompt": "test"},
                "async_mode": True,
            },
        )
    ...
    r, j = await jreq(
        ...
        json={"ref2_image_url": "x", "global_prompt": "t", "duration": 4},
    )  # noqa: E501
    pr(
        cls(r.status_code),
        "BRIDGE",
        "msr-video submit",
        r.status_code,
        r.text[:120] if r.status_code >= 400 else json.dumps(j, ensure_ascii=False)[:120],
    )  # noqa: E501

    # 无限画布桥接：generate 异步提交（无外部服务应优雅失败）
    r, j = await jreq(
        "POST", "/api/generate", json={"type": "image", "prompt": "test", "workflow": "test"}
    )  # noqa: E501
    pr(
        "PASS" if r.status_code < 300 else cls(r.status_code),
        "BRIDGE",
        "/api/generate submit",
        r.status_code,
        r.text[:120],
    ...
    # ---- 清理 ----
    for u in [
        f"/api/director/batches/{b_id}" if b_id else None,
    ...
    s = {
        "total": len(results),
        "PASS": sum(1 for x in results if x["category"] == "PASS"),
        ...
        "SKIP": sum(1 for x in results if x["category"] == "SKIP"),
        "results": results,
    }
    ...
    print("\n===== SUMMARY =====")
    print(
        f"total={s['total']} PASS={s['PASS']} EXPECTED_FAIL={s['EXPECTED_FAIL']} UNEXPECTED={s['UNEXPECTED']} SKIP={s['SKIP']}"  # noqa: E501
    )  # noqa: E501
    return s
