...
S = "x=(W-w)/2:y=H-h-86"
# 无 if 兜底，直接 exp*sin，enable 从 start 起
check(f"{S}+90*exp(-1.6*(t-26.2))*sin((t-26.2)*13):enable=gte(t\\,26.2)", "exp*sin no-if")
# 放大观测：先压到下方再弹，用更明显幅度
check(f"{S}+(90*exp(-1.6*(t-26.2))*sin((t-26.2)*13)):enable=gte(t\\,26.2)", "paren")
